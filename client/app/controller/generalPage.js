    ...
    if($scope.croppedDataUrl) {
      ...
      bucket.putObject(params, function(err, data) {
        if(err) {
          ...
          $scope.data.imgUrl = 'reactorlounge.s3.amazonaws.com/' + $scope.picFile.name
          console.log('Upload Done', $scope.data.imgUrl);
        }
      })
...
//post messages on submit, clear out msg submit field & make a call to initialmsg to fetch msgs
  $scope.postMsg = function(){
    console.log('this is scopeimgUrl', $scope.data.imgUrl)
    generalFeed.addMsg($scope.msg, $scope.data.imgUrl)
    .then(function(){
      $scope.msg=null;
      initialMsgs()
    })
